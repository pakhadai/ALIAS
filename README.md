...
Також для зручності локальної розробки сервер підхоплює змінні з кореневого `.env`, але **`packages/server/.env` має пріоритет** (перезаписує значення з кореня).
...
| `packages/server/.env.example` | Шаблон |
