...
Доступ контролюється сервером і залежить від конфігурації:

- Якщо задано `ADMIN_ALLOWED_EMAILS` (через кому) — **доступ дозволено лише** для JWT-користувачів, чий email входить у whitelist (це має пріоритет).
- Якщо whitelist порожній — fallback на `User.isAdmin === true` у БД (поле в JWT саме по собі недостатнє).
- У production, якщо whitelist порожній і `isAdmin` відсутній — доступ заборонено. У dev можливий fallback-доступ для локальної розробки.
