...
export const ProfileSettingsScreen = () => {
  ...
  return (
    <ScreenShell
      ...
      header={
        <AppHeader
          title={<ScreenTitle themeClass={currentTheme.textMain}>Налаштування профілю</ScreenTitle>}
        ...
        <div className="rounded-2xl bg-ui-card border border-ui-border p-5 space-y-3">
          ...
          {skipNamePrompt && autoNamePreview && (
            <p className={`${typographyClass.label} text-ui-fg-muted`}>
              Буде використано: <span className="font-medium text-ui-fg">{autoNamePreview}</span>
            </p>
          )}
        </div>
      )}
          ...
          {canInstall && (
            <div className="flex justify-between items-center">
              <div className="flex items-center gap-2">
                ...
                <span className={`${typographyClass.system} text-ui-fg`}>На головний екран</span>
              </div>
              <button
                type="button"
                onClick={install}
