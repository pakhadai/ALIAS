...
export function LobbyReadinessBar(props: { readiness: LobbyReadiness; t: T }): React.ReactNode {
  ...
  if (readiness.ok) {
    return (
      ...
      >
        <div
          className={`flex w-full items-center justify-center gap-2 rounded-2xl border border-[color-mix(in_srgb,var(--ui-success)_25%,transparent)] bg-[color-mix(in_srgb,var(--ui-success)_10%,transparent)] px-3 py-2.5 ${systemStatusClass} text-ui-success`}
        >
        ...
        {readiness.hasOverfilledTeams ? (
          <p
            className={`flex items-center justify-center gap-1.5 ${typographyClass.system} text-ui-warning`}
