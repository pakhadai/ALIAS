...
- Liquid Glass a11y: `prefers-reduced-transparency: reduce` hides blur `::before` on header/footer chrome; tint `::after` falls back to opaque gradient without mask (`glass.css`)
- TMA bootstrap: `ready()` / `expand()` run synchronously before first React paint via `bootstrapTelegramMiniApp()` in `index.tsx`; `viewportChanged` re-expands when `isExpanded` is false (`useTelegramApp.ts`)
- Liquid Glass z-index: fixed header/island chrome (`--z-liquid-chrome` 30) no longer collides with modals (`--z-modal` 100) — sheets always stack above glass chrome (`styles.css`, `glass.css`, `constants/zIndex.ts`)
- Liquid Glass: page transition animation no longer leaves a persistent `transform` on the wrapper after `animate-page-in` completes — restores `backdrop-filter` on header/footer chrome (`tailwind.config.ts`, `styles.css`)
...
### Changed
- Mask-based liquid glass chrome for app header and footer: blur `::before` + tint `::after` with vertical mask fade (same pattern as `.bottom-sheet-top-bar`); removed sharp `border-bottom` / `border-top`; feather zone 48px inside layout box (`padding` + negative `margin` on sticky chrome); tint **60% → 25% → transparent** via `--ui-app-header-opacity-*` / `--ui-app-footer-opacity-*` and theme-aware `color-mix` with `--ui-elevated` (`glass.css`, `styles.css`)
- Fixed footer chrome (`FooterIsland`): full-width header-mirror feather from viewport bottom — no floating capsule; shared blur/tint with `.ui-app-footer`; compact CTA row restored (12px bottom gap, feather overlaps scroll via negative margin); glass extends through safe-area + gap reserve (`padding-bottom`, no bottom prosvit)
- Hybrid glass app header: liquid glass base (`backdrop-filter` in `glass.css`) + mask feather `::after` (32px below bar, same `-webkit-mask-image` as `.bottom-sheet-top-bar::after`); `.ui-app-header` `overflow: visible`; deprecated Tailwind `gradient` prop on `GlassAppHeader` (feather is global CSS)
- Liquid Glass Micro B: viewport-fixed `AppHeader` + `FixedBottomBar island` rolled out to menu/lobby screens (`MenuScreen`, `ProfileScreen`, `RulesScreen`, `JoinInputScreen`, `PlayerStatsScreen`, `StoreScreen`, `MyDecksScreen`, `MyWordPacksScreen`, `LobbyScreen`, `SettingsScreen`, `TeamSetupScreen`) and `ScoreboardScreen`; sticky in-flow chrome remains on transitional game-flow screens (`PreRoundScreen`, `VSScreen`, `RoundSummaryScreen`)
- `docs/TMA_LAYOUT.md` synced with Liquid Glass epic (Sessions 0–9): fixed chrome as target (`headerFixed`/`footerFixed`), sticky in-flow as legacy prod; direct blur in `glass.css`; z-index stack; settings screens A+fixed pattern
- Liquid Glass sticky footer: `.ui-app-footer` uses direct `backdrop-filter` in `glass.css` (same pattern as header) — replaces mask-based `::before`/`::after` in `styles.css`; island mode (`FooterIsland`) unchanged
- Liquid Glass fixed chrome on settings screens: `ProfileSettingsScreen` and `LobbySettingsScreen` use viewport-fixed `AppHeader` (`headerFixed`) and floating `FixedBottomBar` island (`footerFixed`) — blur outside scroll container
...
### Added
- **TMA header unification (Phases 0–6):** unified `ScreenShell` + `AppHeader` across menu, profile, lobby-adjacent, and non-playing game screens; `docs/TMA_HEADER_UNIFICATION.md`, header matrix in `docs/TMA_LAYOUT.md`; E2E `@smoke` 15/15
- **Lobby Fix (Phases 1–4 + Final Audit):** `buildTeamShells`, `deriveLobbyReadinessServer`, server join/START guards, i18n, unit/integration/E2E coverage; regression B1–B12 in `AUDIT_RESULTS.md` Block I
- **Lobby redesign (Phases 1–3):** avatar strip, readiness bar, play-mode bar, invite sheet, glass chrome
- **TYPO-001 typography epic (implemented):** semantic `text-ui-*` tokens, `typographyClass` map, `ScreenTitle`, rule `08-typography.mdc`, governance test
...
- **Lobby UX:** invite sheet, play-mode bar, optimistic settings rollback, mid-game join blocked
- **UI tokens (sessions 1–7):** `--ui-*` canon in `docs/UI_TOKENS.md`; admin panel migrated
- **Bundle (C-1):** lazy routes — `main` **~306 KB** / **~94 KB gzip** (was 500 KB)
- **Deploy env:** canonical `.env.prod`; NPM docker network `npm_network` default
- **Doc hygiene (2026-06-09):** consolidated `[Unreleased]`; `AGENT_BRIEF` governance in `CONTRIBUTING` + steward; `docs/archive/README.md`

### Fixed
- **ModalSheet keyboard (all sheets):** built-in `keyboardAvoiding` on every `ModalSheet`; overlap buffer + `pb-4` when keyboard open; stale inset after dismiss; panel scroll reset; removed per-consumer wiring (`EnterNameSheet`, `QuickJoinSheet`, `AddOfflinePlayerSheet`)
- **Lobby:** TMA header gap, play-mode bar slot, scroll/footer overlap, offline remove ghost, `teamsLocked` Redis restore, server readiness validation
- **TMA:** keyboard bottom-sheet drift, MenuScreen header overlap, ModalSheet vs Telegram chrome
- **E2E:** lobby helpers after redesign, offline assign i18n, multiplayer reconnect strict-mode
- **Offline:** `TIME_UP` fallback, `TEAM_JOIN` with empty teams, add-player sheet mount-on-demand

### Removed
- **`docs/LOBBY_FIX_PROMPTS.md`** → `docs/archive/` (epic completed 2026-06-08)
- **E2E debug artifact:** `packages/e2e/debug-offline-add.png`; `.gitignore` `packages/e2e/debug-*.png`
- **ModalSheet `panelVariant`:** deprecated card layout
- **Docs cleanup (2026-06-06):** `docs/Comands.md`, legacy `.cursor/*_AUDIT.md` files

## [2026-04-19] — Release 0.6.2: лобі та правила (UX)

### Changed

- **Налаштування кімнати (`SettingsScreen`)**: вкладка «Правила» — вступний текст, розділи з розгортанням (час і перемога, команди, пропуски; для квізу — типи питань / таймер і помилки); підписи квізу локалізовані (UA / DE / EN).
- **Модальне «Правила» з меню (`RulesModal`)**: спочатку активний режим і кнопка «Усі режими»; швидкий старт і детальні цифри налаштувань — за розгортанням; рядок-зведення для поточних параметрів.
- **Онлайн-лобі (`OnlineLobbyIntro`)**: коротка підказка для обраного режиму (той самий текст, що й підказки режиму в налаштуваннях).
...
- **`.env.example`**: додано/уточнено закоментовані **`ADMIN_*`**, **`TRUST_PROXY_HOPS`**, relay / instance.
...
- **API key authentication for admin:** додано підтримку статичного API-ключа для доступу до адмін-функцій як альтернатива JWT-токенам. Це дозволяє входити в адмінку без авторизації через Google/Apple. Файли: `packages/server/src/config.ts` (нова змінна `adminApiKey`), `packages/server/src/routes/admin.ts` (оновлена функція `adminAuth`), `.env` (нова змінна `ADMIN_API_KEY`).
...
### Fixed
- **"Admin access is not configured" error:** виправлено помилку, що виникала при спробі доступу до адмінки в продакшені без налаштованих дозволених email. Тепер система правильно перевіряє конфігурацію та дозволяє доступ через API-ключ. Файли: `packages/server/src/routes/admin.ts`, `.env` (додано `ADMIN_ALLOWED_EMAILS`).
